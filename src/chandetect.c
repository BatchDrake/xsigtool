...
#define peak_detector_INITIALIZER       \
{                                       \
  0, /* size */                         \
  0, /* thr2 */                         \
  NULL, /* history */                   \
...
typedef struct peak_detector peak_detector_t;

SUBOOL
peak_detector_init(peak_detector_t *pd, unsigned int size, SUFLOAT thres)
{
  SUFLOAT *history;
...
int
peak_detector_feed(peak_detector_t *pd, SUFLOAT x)
{
  SUFLOAT mean;
  SUFLOAT variance = .0;
  SUFLOAT d;
...
void
peak_detector_finalize(peak_detector_t *pd)
{
  if (pd->history != NULL)
    free(pd->history);
}
